...
//                        System.out.println(bucket1);
    ...
    @Override
    public Orders placeOrders(String token, PaymentDto payment) throws CustomerException {
        CustomerLogin customerLogin = this.checkPermission(token);
        if(customerLogin != null){
           Optional<Customer> optionalCustomer = customerRepo.findById(customerLogin.getCustomerId());
            if(optionalCustomer.get().getBucket() != null){
                Orders order = new Orders();
                order.setOrderDate(LocalDate.now());
                order.setQuantity(optionalCustomer.get().getBucket().getTotalItems());
                order.setTotalCost(optionalCustomer.get().getBucket().getTotalPrice());
                order.setTransactionMode("Card");

                Payment payment1 = new Payment();
                payment1.setCVV(payment.getCVV());
                payment1.setCardNumber(payment.getCardNumber());
                payment1.setExpiryDate(payment.getExpiryDate());
                payment1.setCustomerId(customerLogin.getCustomerId());
                payment1.setMobileNumber(payment.getMobileNumber());

//                paymentRepo.save(payment1);
                Orders orders1= orderReop.save(order);
                optionalCustomer.get().getBucket().setOrders(orders1);
                Bucket bucket= bucketRepo.save(optionalCustomer.get().getBucket());

                optionalCustomer.get().setBucket(bucket);
                customerRepo.save(optionalCustomer.get());
                return orders1;

            }else{
                throw new CustomerException("Your Cart isEmpty");
            }
        ...
        throw new CustomerException("Customer Key is not Valid");
